...
func (c *AppController) List(ctx *gin.Context) {
	...
	if bundleID := ctx.Query("bundle_id"); bundleID != "" {
...
func (c *AppController) Create(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusCreated, gin.H{"code": 0, "message": "创建成功", "data": app})
}
...
func (c *AppController) Delete(ctx *gin.Context) {
	id := ctx.Param("id")
	if err := c.DB.Delete(&models.App{}, id).Error; err != nil {
...
func (c *AppController) ListVersions(ctx *gin.Context) {
	...
	var total int64
	query.Model(&models.AppVersion{}).Count(&total)
...
func (c *AppController) CreateVersion(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusCreated, gin.H{"code": 0, "message": "创建成功", "data": version})
}

// DeleteVersion 删除应用版本
func (c *AppController) DeleteVersion(ctx *gin.Context) {
	versionID := ctx.Param("version_id")
	if err := c.DB.Delete(&models.AppVersion{}, versionID).Error; err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": "删除失败"})
		return
	}
	ctx.JSON(http.StatusOK, gin.H{"code": 0, "message": "删除成功"})
...
func (c *AppController) ListDistributions(ctx *gin.Context) {
	...
	if err := query.Order("created_at DESC").Offset(offset).Limit(pageSize).Find(&distributions).Error; err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": "查询失败"})
		return
	}
...
func (c *AppController) CreateDistribution(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusCreated, gin.H{"code": 0, "message": "创建成功", "data": dist})
}

// GetDistribution 获取分发任务详情
func (c *AppController) GetDistribution(ctx *gin.Context) {
	...
	if err := c.DB.First(&dist, id).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			ctx.JSON(http.StatusNotFound, gin.H{"code": 404, "message": "分发任务不存在"})
			return
		}
		ctx.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": "查询失败"})
		return
	}
	ctx.JSON(http.StatusOK, gin.H{"code": 0, "message": "success", "data": dist})
}
...
func (c *AppController) CancelDistribution(ctx *gin.Context) {
	...
	if dist.Status != "pending" && dist.Status != "running" {
		ctx.JSON(http.StatusBadRequest, gin.H{"code": 400, "message": "当前状态无法取消"})
		return
	}

	if err := c.DB.Model(&dist).Update("status", "cancelled").Error; err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"code": 500, "message": "取消失败"})
		return
	}

	ctx.JSON(http.StatusOK, gin.H{"code": 0, "message": "取消成功"})
}
...
func (c *AppController) GetStats(ctx *gin.Context) {
	...
	c.DB.Model(&models.AppVersion{}).Where("app_id = ?", app.ID).Count(&versionCount)

	var distCount int64
	c.DB.Model(&models.AppDistribution{}).Where("app_id = ?", app.ID).Count(&distCount)

	var installCount int64
	c.DB.Model(&models.AppInstallRecord{}).Where("app_id = ?", app.ID).Count(&installCount)
