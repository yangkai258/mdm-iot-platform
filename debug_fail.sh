...
curl -v -X POST "http://localhost:8080/api/v1/orders" -H "$AUTH" -H "Content-Type: application/json" -d '{"total_amount":100}' 2>&1

echo ""
echo "=== Testing POST /knowledge/query ==="
curl -v -X POST "http://localhost:8080/api/v1/knowledge/query" -H "$AUTH" -H "Content-Type: application/json" -d '{"query":"test","top_k":5}' 2>&1
