...
func RegisterRoutes(r *gin.Engine, db *gorm.DB, redisClient *utils.RedisClient) {
	...
	api := r.Group("/api/v1")
	{
		...
		// 注意：/:id路由必须放在具体路径后面
		api.GET("/members/coupons", memberCtrl.CouponList)
		api.POST("/members/coupons", memberCtrl.CouponCreate)
		api.PUT("/members/coupons/:id", memberCtrl.CouponUpdate)
		api.DELETE("/members/coupons/:id", memberCtrl.CouponDelete)
		api.GET("/members/cards", memberCtrl.CardList)
