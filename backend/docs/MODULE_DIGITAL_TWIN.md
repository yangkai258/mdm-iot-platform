...
### 6.1 数字孪生首页（/digital-twin/:pet_id）

**布局结构：**
1. 面包屑 → 页面标题 + 设备选择
2. 宠物3D虚拟形象展示区（白色，大卡片）
3. 实时体征面板（心跳/呼吸/体温/情绪指数）—— 白色卡片
4. 快捷操作区（查看体征/健康预警/时间轴）

**非标准说明：** 本页面为可视化展示型布局，采用大卡片网格布局，而非标准三段式。

### 6.2 实时体征面板（/digital-twin/:pet_id/vitals）

**布局结构：**
1. 面包屑 → 页面标题 + 设备信息
2. 体征统计卡片（当前心跳/呼吸频率/体温）—— 白色
3. 曲线图表（心跳曲线/呼吸曲线/体温曲线 Tab）
4. 实时数据列表

**分页：** 右下角，10/20/50/100 条

### 6.3 健康预警页面（/digital-twin/:pet_id/alerts）

**布局结构：**
1. 面包屑 → 页面标题
2. 筛选区（浅灰卡片 #F2F3F5）：预警类型 / 严重程度 / 状态 / 日期范围
3. 操作栏（导出报告靠右）
4. 健康预警列表表格
...
| 触发值 | 100px | - |
| 阈值 | 100px | - |
| 严重程度 | 100px | info/warning/critical |
| 状态 | 100px | pending/acknowledged/ignored/resolved |
| 创建时间 | 150px | - |
| 操作 | 120px | 详情/确认/忽略 |

**分页：** 右下角，10/20/50/100 条

### 6.4 时间轴页面（/digital-twin/:pet_id/timeline）

**布局结构：**
1. 面包屑 → 页面标题
2. 筛选区（浅灰卡片）：事件类型 / 日期范围
3. 时间轴组件（按日/周/月视图切换）
4. 事件列表（时间倒序）

**分页：** 右下角，10/20/50/100 条

### 6.5 回忆集页面（/digital-twin/:pet_id/memories）
...
### 6.6 跨设备同步页面（/digital-twin/:pet_id/sync）
...
| 序号 | 60px | - |
| 设备ID | 150px | - |
| 同步类型 | 100px | push/pull/batch |
| 状态 | 100px | success/failed/partial |
| 同步数量 | 100px | - |
| 同步时间 | 150px | - |
