<template>
  <div class="pro-page-container">
    ...
    <div class="pro-action-bar">
      <a-space>
        <a-button type="primary" @click="openCreate">新增临时会员</a-button>
    ...
    <a-modal v-model:visible="modalVisible" title="新增临时会员" @ok="handleSubmit" :width="480" :mask-closable="false">
...
const form = reactive({ name: '', phone: '', gender: '1', store_id: undefined, remark: '' })
...
const openCreate = () => { Object.assign(form, { name: '', phone: '', gender: '1', store_id: undefined, remark: '' }); modalVisible.value = true }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const data = await res.json()
    if (data.code === 0) { Message.success('创建成功'); modalVisible.value = false; loadData() }
...
const convertToFormal = async (r) => {
  ...
  try {
    ...
    const data = await res.json()
    if (data.code === 0) { Message.success('转化成功'); loadData() } else Message.error(data.message || '转化失败')
