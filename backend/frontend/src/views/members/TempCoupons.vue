...
const loadCoupons = async () => {
  try {
    ...
    if (data.code === 0) coupons.value = data.data?.list || []
  } catch (e) {}
}
...
const loadTempMembers = async () => {
  try {
    ...
    const data = await res.json()
    if (data.code === 0) tempMembers.value = data.data?.list || []
  } catch (e) {}
}
...
const handleIssue = async () => {
  ...
  try {
    ...
    const data = await res.json()
    if (data.code === 0) { Message.success('发放成功'); openIssueModal.value = false; loadData() }
    else Message.error(data.message || '发放失败')
