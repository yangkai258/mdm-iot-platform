...
func RegisterRoutes(r *gin.Engine, db *gorm.DB, redisClient *utils.RedisClient) {
	...
	api := r.Group("/api/v1")
	{
		...
		// ============ 保险模块 ============
		insuranceCtrl := &InsuranceController{DB: db}
		api.GET("/insurance/products", insuranceCtrl.InsuranceProductList)
		api.GET("/insurance/products/:id", insuranceCtrl.InsuranceProductGet)
		api.POST("/insurance/products", insuranceCtrl.InsuranceProductCreate)
		api.PUT("/insurance/products/:id", insuranceCtrl.InsuranceProductUpdate)
		api.DELETE("/insurance/products/:id", insuranceCtrl.InsuranceProductDelete)
		api.GET("/insurance/claims", insuranceCtrl.InsuranceClaimList)
		api.GET("/insurance/claims/:id", insuranceCtrl.InsuranceClaimGet)
		api.POST("/insurance/claims", insuranceCtrl.InsuranceClaimCreate)
		api.PUT("/insurance/claims/:id", insuranceCtrl.InsuranceClaimUpdate)
		api.DELETE("/insurance/claims/:id", insuranceCtrl.InsuranceClaimDelete)
		api.POST("/insurance/claims/:id/review", insuranceCtrl.InsuranceClaimReview)
		api.GET("/insurance/documents", insuranceCtrl.InsuranceClaimDocumentList)
		api.POST("/insurance/documents", insuranceCtrl.InsuranceClaimDocumentCreate)
		api.DELETE("/insurance/documents/:id", insuranceCtrl.InsuranceClaimDocumentDelete)

		// ============ 情感模块 ============
		emotionCtrl := &EmotionController{DB: db}
		api.GET("/emotion/records", emotionCtrl.EmotionRecordList)
		api.GET("/emotion/records/:id", emotionCtrl.EmotionRecordGet)
		api.POST("/emotion/records", emotionCtrl.EmotionRecordCreate)
		api.DELETE("/emotion/records/:id", emotionCtrl.EmotionRecordDelete)
		api.GET("/emotion/reports", emotionCtrl.EmotionReportList)
		api.GET("/emotion/reports/:id", emotionCtrl.EmotionReportGet)
		api.POST("/emotion/reports", emotionCtrl.EmotionReportCreate)
		api.DELETE("/emotion/reports/:id", emotionCtrl.EmotionReportDelete)
		api.GET("/emotion/configs", emotionCtrl.EmotionResponseConfigList)
		api.GET("/emotion/configs/:id", emotionCtrl.EmotionResponseConfigGet)
		api.POST("/emotion/configs", emotionCtrl.EmotionResponseConfigCreate)
		api.PUT("/emotion/configs/:id", emotionCtrl.EmotionResponseConfigUpdate)
		api.DELETE("/emotion/configs/:id", emotionCtrl.EmotionResponseConfigDelete)

		// ============ 健康模块 ============
		healthCtrl := &HealthController{DB: db}
		api.GET("/health/warnings", healthCtrl.HealthWarningList)
		api.GET("/health/warnings/:id", healthCtrl.HealthWarningGet)
		api.POST("/health/warnings", healthCtrl.HealthWarningCreate)
		api.PUT("/health/warnings/:id", healthCtrl.HealthWarningUpdate)
		api.POST("/health/warnings/:id/resolve", healthCtrl.HealthWarningResolve)
		api.DELETE("/health/warnings/:id", healthCtrl.HealthWarningDelete)
		api.GET("/health/vitals", healthCtrl.VitalRecordList)
		api.GET("/health/vitals/:id", healthCtrl.VitalRecordGet)
		api.POST("/health/vitals", healthCtrl.VitalRecordCreate)
		api.DELETE("/health/vitals/:id", healthCtrl.VitalRecordDelete)
		api.GET("/health/vitals/trends", healthCtrl.VitalTrendList)
		api.GET("/health/exercise/goals", healthCtrl.ExerciseGoalList)
		api.GET("/health/exercise/goals/:id", healthCtrl.ExerciseGoalGet)
		api.POST("/health/exercise/goals", healthCtrl.ExerciseGoalCreate)
		api.PUT("/health/exercise/goals/:id", healthCtrl.ExerciseGoalUpdate)
		api.DELETE("/health/exercise/goals/:id", healthCtrl.ExerciseGoalDelete)
		api.GET("/health/exercise/records", healthCtrl.ExerciseRecordList)
		api.GET("/health/exercise/records/:id", healthCtrl.ExerciseRecordGet)
		api.POST("/health/exercise/records", healthCtrl.ExerciseRecordCreate)
		api.DELETE("/health/exercise/records/:id", healthCtrl.ExerciseRecordDelete)
		api.GET("/health/exercise/summaries", healthCtrl.ExerciseSummaryList)
		api.GET("/health/exercise/trends", healthCtrl.ExerciseTrendList)
		api.GET("/health/alert-rules", healthCtrl.HealthAlertRuleList)
		api.GET("/health/alert-rules/:id", healthCtrl.HealthAlertRuleGet)
		api.POST("/health/alert-rules", healthCtrl.HealthAlertRuleCreate)
		api.PUT("/health/alert-rules/:id", healthCtrl.HealthAlertRuleUpdate)
		api.DELETE("/health/alert-rules/:id", healthCtrl.HealthAlertRuleDelete)
		api.GET("/health/monitor-settings", healthCtrl.HealthMonitorSettingList)
		api.GET("/health/monitor-settings/:id", healthCtrl.HealthMonitorSettingGet)
		api.POST("/health/monitor-settings", healthCtrl.HealthMonitorSettingCreate)
		api.PUT("/health/monitor-settings/:id", healthCtrl.HealthMonitorSettingUpdate)
		api.GET("/health/alerts", healthCtrl.HealthAlertList)
		api.GET("/health/alerts/:id", healthCtrl.HealthAlertGet)
		api.POST("/health/alerts", healthCtrl.HealthAlertCreate)
		api.POST("/health/alerts/:id/resolve", healthCtrl.HealthAlertResolve)
		api.DELETE("/health/alerts/:id", healthCtrl.HealthAlertDelete)
		api.GET("/health/vaccinations", healthCtrl.PetVaccinationList)
		api.POST("/health/vaccinations", healthCtrl.PetVaccinationCreate)
		api.DELETE("/health/vaccinations/:id", healthCtrl.PetVaccinationDelete)
		api.GET("/health/diet-records", healthCtrl.PetDietRecordList)
		api.POST("/health/diet-records", healthCtrl.PetDietRecordCreate)
		api.DELETE("/health/diet-records/:id", healthCtrl.PetDietRecordDelete)
		api.GET("/health/pet-records", healthCtrl.PetHealthRecordList)
		api.GET("/health/pet-records/:id", healthCtrl.PetHealthRecordGet)
		api.POST("/health/pet-records", healthCtrl.PetHealthRecordCreate)
		api.PUT("/health/pet-records/:id", healthCtrl.PetHealthRecordUpdate)
		api.DELETE("/health/pet-records/:id", healthCtrl.PetHealthRecordDelete)

		// ============ 宠物商店模块 ============
		petShopCtrl := &PetShopController{DB: db}
		api.GET("/pet-shop/products", petShopCtrl.PetShopProductList)
		api.GET("/pet-shop/products/:id", petShopCtrl.PetShopProductGet)
		api.POST("/pet-shop/products", petShopCtrl.PetShopProductCreate)
		api.PUT("/pet-shop/products/:id", petShopCtrl.PetShopProductUpdate)
		api.DELETE("/pet-shop/products/:id", petShopCtrl.PetShopProductDelete)
		api.GET("/pet-shop/orders", petShopCtrl.PetShopOrderList)
		api.GET("/pet-shop/orders/:id", petShopCtrl.PetShopOrderGet)
		api.POST("/pet-shop/orders", petShopCtrl.PetShopOrderCreate)
		api.PUT("/pet-shop/orders/:id", petShopCtrl.PetShopOrderUpdate)
		api.DELETE("/pet-shop/orders/:id", petShopCtrl.PetShopOrderDelete)
		api.GET("/pet-shop/cart", petShopCtrl.PetShopCartList)
		api.POST("/pet-shop/cart/add", petShopCtrl.PetShopCartAdd)
		api.DELETE("/pet-shop/cart/:id", petShopCtrl.PetShopCartRemove)
		api.GET("/pet-shop/favorites", petShopCtrl.PetShopFavoriteList)
		api.POST("/pet-shop/favorites/add", petShopCtrl.PetShopFavoriteAdd)
		api.DELETE("/pet-shop/favorites/:id", petShopCtrl.PetShopFavoriteRemove)

		// ============ 寻宠网络模块 ============
		petFinderCtrl := &PetFinderController{DB: db}
		api.GET("/pet-finder/reports", petFinderCtrl.PetFinderReportList)
		api.GET("/pet-finder/reports/:id", petFinderCtrl.PetFinderReportGet)
		api.POST("/pet-finder/reports", petFinderCtrl.PetFinderReportCreate)
		api.PUT("/pet-finder/reports/:id", petFinderCtrl.PetFinderReportUpdate)
		api.POST("/pet-finder/reports/:id/resolve", petFinderCtrl.PetFinderReportResolve)
		api.DELETE("/pet-finder/reports/:id", petFinderCtrl.PetFinderReportDelete)
		api.GET("/pet-finder/sightings", petFinderCtrl.PetFinderSightingList)
		api.POST("/pet-finder/sightings", petFinderCtrl.PetFinderSightingCreate)
		api.POST("/pet-finder/sightings/:id/verify", petFinderCtrl.PetFinderSightingVerify)
		api.GET("/pet-finder/alerts", petFinderCtrl.PetFinderAlertList)
		api.GET("/pet-finder/sighting-reports", petFinderCtrl.SightingReportList)
		api.POST("/pet-finder/sighting-reports", petFinderCtrl.SightingReportCreate)
		api.DELETE("/pet-finder/sighting-reports/:id", petFinderCtrl.SightingReportDelete)

		// ============ BLE Mesh 模块 ============
		meshCtrl := &MeshController{DB: db}
		api.GET("/mesh/networks", meshCtrl.MeshNetworkList)
		api.GET("/mesh/networks/:id", meshCtrl.MeshNetworkGet)
		api.POST("/mesh/networks", meshCtrl.MeshNetworkCreate)
		api.PUT("/mesh/networks/:id", meshCtrl.MeshNetworkUpdate)
		api.DELETE("/mesh/networks/:id", meshCtrl.MeshNetworkDelete)
		api.GET("/mesh/nodes", meshCtrl.MeshNodeList)
		api.GET("/mesh/nodes/:id", meshCtrl.MeshNodeGet)
		api.POST("/mesh/nodes", meshCtrl.MeshNodeCreate)
		api.PUT("/mesh/nodes/:id", meshCtrl.MeshNodeUpdate)
		api.DELETE("/mesh/nodes/:id", meshCtrl.MeshNodeDelete)
		api.GET("/mesh/groups", meshCtrl.MeshGroupList)
		api.POST("/mesh/groups", meshCtrl.MeshGroupCreate)
		api.DELETE("/mesh/groups/:id", meshCtrl.MeshGroupDelete)
		api.GET("/mesh/node-groups", meshCtrl.MeshNodeGroupList)
		api.POST("/mesh/node-groups/add", meshCtrl.MeshNodeGroupAdd)
		api.DELETE("/mesh/node-groups/remove", meshCtrl.MeshNodeGroupRemove)
		api.GET("/mesh/telemetry", meshCtrl.MeshTelemetryList)
		api.POST("/mesh/telemetry", meshCtrl.MeshTelemetryCreate)

		// ============ 国际化模块 ============
		i18nCtrl := &I18nController{DB: db}
		api.GET("/i18n/translations", i18nCtrl.TranslationList)
		api.GET("/i18n/translations/:id", i18nCtrl.TranslationGet)
		api.POST("/i18n/translations", i18nCtrl.TranslationCreate)
		api.PUT("/i18n/translations/:id", i18nCtrl.TranslationUpdate)
		api.DELETE("/i18n/translations/:id", i18nCtrl.TranslationDelete)
		api.POST("/i18n/translations/batch", i18nCtrl.TranslationBatchCreate)
		api.GET("/i18n/locale/:locale", i18nCtrl.TranslationByLocale)
		api.GET("/i18n/namespaces", i18nCtrl.TranslationNamespaceList)
		api.GET("/i18n/locales", i18nCtrl.TranslationLocaleList)

		// ============ 区域模块 ============
		regionCtrl := &RegionController{}
		api.GET("/regions/provinces", regionCtrl.RegionProvinceList)
		api.GET("/regions/cities", regionCtrl.RegionCityList)
		api.GET("/regions/districts", regionCtrl.RegionDistrictList)
		api.GET("/regions/search", regionCtrl.RegionSearch)
	}
}
