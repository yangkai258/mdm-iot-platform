      ...
      <a-col :xs="24" :sm="12" :lg="6">
        <a-card class="stat-card">
          <a-statistic
            ...
            :value-style="{ color: '#1650ff' }"
          >
            <template #suffix>
              <span class="stat-suffix">台</span>
            </template>
            <template #prefix>
              <icon-desktop style="margin-right: 8px;" />
            </template>
          </a-statistic>
        </a-card>
      </a-col>
      ...
      <a-col :xs="24" :sm="12" :lg="6">
        <a-card class="stat-card">
          ...
          >
            ...
            <template #prefix>
              <icon-alert style="margin-right: 8px;" />
            </template>
          </a-statistic>
        </a-card>
      </a-col>
    ...
    <a-row :gutter="[16, 16]" style="margin-top: 16px;">
      <a-col :span="24">
        <a-card title="快捷入口" class="quick-access-card">
          <template #extra>
            <a-link @click="$router.push('/portal')">更多</a-link>
          </template>
    ...
    <a-row :gutter="[16, 16]" style="margin-top: 16px;">
      ...
      <a-col :xs="24" :lg="12">
        <a-card title="系统广播" class="recent-card">
          <template #extra>
            <a-link @click="$router.push('/broadcast')">查看全部</a-link>
          </template>
          <a-list :data="recentBroadcasts" :bordered="false">
            <template #empty>
              <a-empty description="暂无广播" />
            </template>
            <template #item="{ item }">
              <a-list-item>
                <a-list-item-meta
                ...
                >
                  <template #avatar>
                    <a-avatar :style="{ backgroundColor: '#1650ff' }">
                      <icon-broadcast />
                    </a-avatar>
                  </template>
                </a-list-item-meta>
...
const fetchDashboardData = async () => {
  try {
    ...
    if (devicesRes.data.code === 0) {
      ...
      const onlineCount = devicesRes.data.data.list?.filter(d => d.is_online).length || 0
    ...
    if (broadcastsRes.data.code === 0) {
      recentBroadcasts.value = broadcastsRes.data.data.list || []
    }
  } catch (error) {
