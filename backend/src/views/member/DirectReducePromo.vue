<template>
  <div class="direct-reduce-promo-page">
    <a-breadcrumb class="breadcrumb">
      ...
      <a-breadcrumb-item>直减促销</a-breadcrumb-item>
    </a-breadcrumb>
    ...
    <a-row :gutter="16" style="margin-bottom: 16px;">
      ...
      <a-col :span="8"><a-card hoverable><a-statistic title="优惠金额总计" :value="(stats.totalSaved || 0).toLocaleString() + ' 元'" /></a-card></a-col>
    </a-row>
    ...
    <a-modal v-model:visible="formVisible" :title="isEdit ? '编辑活动' : '新建活动'"
      ...
      <a-form :model="form" layout="vertical">
        <a-form-item label="活动名称" :rules="[{ required: true, message: '请输入活动名称' }]">
          <a-input v-model="form.name" placeholder="如：夏日清凉直减" />
        </a-form-item>
        <a-form-item label="直减金额(元)" :rules="[{ required: true, message: '请输入直减金额' }]">
          <a-input-number v-model="form.reduceAmount" :min="0.01" :precision="2" style="width: 100%" />
        </a-form-item>
...
const form = reactive({ name: '', reduceAmount: 5, minAmount: 0, startTime: null, endTime: null, status: 'active' })

const columns = [
  { title: '活动名称', slotName: 'name', width: 180 },
  { title: '直减金额', slotName: 'reduceAmount', width: 120 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await api.getDirectReducePromoList()
...
const showCreate = () => { isEdit.value = false; Object.assign(form, { name: '', reduceAmount: 5, minAmount: 0, startTime: null, endTime: null, status: 'active' }); formVisible.value = true }
...
const handleFormSubmit = async (done) => {
  ...
  try {
    if (isEdit.value) await api.updateDirectReducePromo(currentId.value, { ...form })
    else await api.createDirectReducePromo({ ...form })
...
const handleDelete = (record) => {
  Modal.warning({ title: '确认删除', content: `确定删除活动「${record.name}」吗？`, okText: '确认删除',
    onOk: async () => { try { await api.deleteDirectReducePromo(record.id); Message.success('删除成功'); loadData() } catch (err) { Message.error(err.message || '删除失败') } }
  })
}
...
<style scoped>
.direct-reduce-promo-page { padding: 20px 24px; min-height: calc(100vh - 64px); background: #f5f7fa; }
