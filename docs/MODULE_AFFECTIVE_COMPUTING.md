...
### 6.2 响应策略配置页面（/emotion/:pet_id/response-config）

**布局结构：**
1. 面包屑 → 页面标题 + 宠物信息
2. 配置表单区（灵敏度/响应延迟/响应频率/禁用动作）—— 浅灰卡片
3. 动作库列表
...
### 6.3 情绪日志页面（/emotion/logs）

**布局结构：**
1. 面包屑 → 页面标题
2. 筛选区（浅灰卡片 #F2F3F5）：情绪类型 / 来源 / 日期范围 / 关键词
3. 操作栏（导出靠右）
4. 情绪日志列表（卡片形式）
...
### 6.4 家庭情绪地图页面（/emotion/family/:family_id）

**布局结构：**
1. 面包屑 → 页面标题 + 家庭信息
2. 家庭整体情绪指数卡片（白色）
3. 家庭成员情绪状态（头像+情绪标签网格）
4. 情绪关系图（可视化图形）
5. 情绪建议列表
