...
**布局结构：**
1. 面包屑 → 页面标题
2. 筛选区（浅灰卡片 #F2F3F5）：设备 / 类型 / 状态 / 严重程度
3. 操作栏（批量确认/批量解决靠右）
4. 告警记录列表表格
...
**布局结构：**
1. 面包屑 → 页面标题
2. 统计卡片（今日告警/待处理/严重/已解决）—— 白色
3. 图表区域（趋势图/分布饼图/设备TOP10）
4. 告警列表表格
...
| 类型 | 使用场景 |
|------|----------|
| Drawer 抽屉 | 新建/编辑告警规则、告警详情 |
| Dialog 对话框 | 确认告警、解决告警、批量确认/解决 |
| 全屏模态 | 暂无复杂表单场景 |
