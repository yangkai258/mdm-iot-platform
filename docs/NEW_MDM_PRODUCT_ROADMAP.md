...
| **设备使用统计**（时长/频率） | P2 | MODULE_DEVICE_MANAGEMENT.md |
...
| **告警抑制/去重**（相同告警合并） | P2 | MODULE_ALERT_SYSTEM.md |
...
| **会员等级权益配置**（折扣/积分倍率/特权） | P1 | MODULE_MEMBER_MANAGEMENT.md |
| **会员升级动画/特效** | P2 | MODULE_MEMBER_MANAGEMENT.md |
| **会员活跃度分析**（留存/转化漏斗） | P2 | MODULE_MEMBER_MANAGEMENT.md |
| **会员360度画像** | P2 | MODULE_MEMBER_MANAGEMENT.md |
...
| **订阅自动续费扣款** | P1 | MODULE_SUBSCRIPTION.md |
...
| 状态 | 说明 | 允许转换到 |
|------|------|-----------|
| 1-待激活 | 设备已注册但未完成首次配对激活 | 2-服役中 |
| 2-服役中 | 设备正常运行，被用户绑定使用 | 1-待激活、3-维修中、4-已挂失、5-已报废 |
| 3-维修中 | 设备寄回维修，无法使用 | 2-服役中、5-已报废 |
| 4-已挂失 | 设备丢失，已挂失处理 | 2-服役中（找回）、5-已报废 |
| 5-已报废 | 设备永久停用，不可再激活 | — |
...
| 状态 | 说明 |
|------|------|
| pending | 部署任务已创建，等待调度 |
| running | 部署进行中，设备逐批升级 |
| paused | 部署暂停，可恢复 |
| completed | 部署全部完成 |
| failed | 部署失败（部分设备升级失败） |
| cancelled | 部署取消 |
...
> **说明：** 以下 MQTT 协议规范基于 MODULE_MINICLAW_PROTOCOL.md，定义设备与云端之间的通信协议结构。
...
| 字段 | 说明 |
|------|------|
| `device_id` | 设备唯一标识（UUID 或设备序列号） |
| `direction` | 通信方向：`up`（设备→云端）或 `down`（云端→设备） |
| `type` | 消息类型：voice/sensor/status/ack/action/speech/config 等 |

### 11.2 上行 Topic（设备→云端）

| Topic | 描述 | QoS | Payload 示例 |
|-------|------|-----|--------------|
| `/miniclaw/{device_id}/up/voice` | 语音识别结果上报 | 1 | `{"text":"今天天气怎么样","emotion":"neutral"}` |
| `/miniclaw/{device_id}/up/sensor` | 传感器数据上报 | 1 | `{"temperature":25.5,"humidity":60,"battery":85}` |
| `/miniclaw/{device_id}/up/status` | 设备状态上报（心跳） | 1 | `{"online":true,"signal":-65,"firmware":"v1.2.3"}` |
| `/miniclaw/{device_id}/up/ack` | 命令执行结果确认 | 1 | `{"cmd_id":"xxx","status":"success","result":{}}` |

### 11.3 下行 Topic（云端→设备）

| Topic | 描述 | QoS | Payload 示例 |
|-------|------|-----|--------------|
| `/miniclaw/{device_id}/down/action` | 云端动作指令下发 | 1 | `{"cmd_id":"xxx","action":"speak","params":{"text":"你好"}}` |
| `/miniclaw/{device_id}/down/speech` | 云端语音内容下发（TTS） | 1 | `{"text":"我是你的AI宠物","voice_id":"xiaoyan"}` |
| `/miniclaw/{device_id}/down/config` | 云端配置参数下发（设备影子） | 1 | `{"desired_state":{"volume":80,"mode":"active"}}` |

### 11.4 通用 Topic（广播）

| Topic | 描述 | QoS | 适用场景 |
|-------|------|-----|----------|
| `/miniclaw/broadcast/system` | 系统广播消息（全员推送） | 1 | 系统公告、维护通知 |
| `/miniclaw/broadcast/ota` | OTA 全量推送 | 1 | 固件全量升级 |

### 11.5 协议帧格式

所有 MQTT 消息 Payload 采用以下二进制帧格式：
...
| 字段 | 长度 | 说明 |
|------|------|------|
| 帧头 | 2B | 固定值 `0xMC`（MiniClaw 标识） |
| 长度 | 2B | 帧总长度（网络序，Big Endian） |
| 版本 | 1B | 协议版本号（当前 `0x01`） |
| 类型 | 1B | 消息类型：`0x01`=语音, `0x02`=传感器, `0x03`=状态, `0x04`=ACK, `0x10`=动作, `0x11`=语音合成, `0x12`=配置 |
| 序列号 | 4B | 消息唯一序列号（用于 ACK 匹配） |
| Payload | N | 业务数据（JSON 格式） |
| 校验 | 2B | CRC16 校验（覆盖帧头到 Payload） |

### 11.6 QoS 级别说明

| QoS 级别 | 说明 | 使用场景 |
|----------|------|----------|
| QoS 0 | 只发送一次，不确认 | 传感器高频数据上报 |
| QoS 1 | 至少发送一次，必须 ACK | 指令下发、配置变更、OTA 推送 |
| QoS 2 | 恰好一次发送 | 支付指令、重要配置（Phase 2 启用） |

### 11.7 设备心跳机制

- **心跳间隔：** 设备每 60 秒发送一次 `/up/status`
- **心跳超时：** 云端 90 秒未收到心跳，标记设备为离线
- **Redis 设备影子 TTL：** 90 秒，自动续期

### 11.8 MQTT 连接安全

- **认证方式：** TLS + X.509 客户端证书双向认证
- **Topic 权限：** 设备只能发布到 `/miniclaw/{device_id}/up/*`，订阅 `/miniclaw/{device_id}/down/*`
- **ACL 规则：** EMQX 访问控制列表限制 Topic 访问范围
...
| 等级 | 颜色名称 | Hex 值 | 使用场景 |
|------|----------|--------|----------|
| 普通 | 灰 | `#909399` | 等级徽章、标签背景 |
| 白银 | 蓝 | `#409EFF` | 等级徽章、标签背景、升级提示 |
| 黄金 | 黄 | `#E6A23C` | 等级徽章、标签背景、升级提示 |
| 铂金 | 紫 | `#9C27B0` | 等级徽章、标签背景、特权标识 |
| 钻石 | 金红 | `#F56C6C` | 等级徽章、标签背景、高光特效 |

#### 会员状态颜色规范

| 状态 | 颜色名称 | Hex 值 | 使用场景 |
|------|----------|--------|----------|
| 正常 | 绿色 | `#67C23A` | 状态指示灯、启用状态、在线设备 |
| 禁用 | 红色 | `#F56C6C` | 禁用状态指示灯、离线设备、错误提示 |
| 警告 | 橙色 | `#E6A23C` | 告警状态、即将到期提醒 |
| 未知 | 灰色 | `#909399` | 未知状态、设备未激活 |
...
#### 设备使用统计页面
- **页面路径**: `/devices/:id/usage-stats`
- **页面布局**: 图表式（顶部 KPI 卡片 + 下方趋势图）
- **核心组件**: 当日/本周/本月使用时长卡片、使用趋势折线图、会话次数柱状图、平均响应时长
- **交互规范**: 时间范围切换（今日/本周/本月/自定义）；点击图表钻取详情；支持数据导出
- **按钮**: [时间范围] 靠左, [导出数据] 靠右
...
#### 告警抑制规则页面
- **页面路径**: `/alerts/deduplication-rules`
- **页面布局**: 规则列表 + 表单抽屉
- **核心组件**: 抑制规则列表（规则名称/条件/抑制时长）、规则启用状态、命中统计
...
#### 会员等级权益配置页面
- **页面路径**: `/members/levels/benefits`
- **页面布局**: 表格 + 权益预览面板
- **核心组件**: 等级列表（等级名/升级门槛/权益）、折扣比例/积分倍率/特权标签、权益预览卡片
- **交互规范**: 权益项可编辑（折扣/积分倍率/专属特权）；批量保存；权益冲突提示
- **按钮**: [编辑权益] 靠左, [预览效果] 靠右

#### 会员升级动画配置页面
- **页面路径**: `/members/levels/upgrade-animation`
- **页面布局**: 预览 + 配置双栏
- **核心组件**: 动画效果选择（光效/粒子/徽章）、动画时长/音效配置、升级触发时机设置
- **交互规范**: 实时预览动画效果；支持自定义动画素材上传；动画性能影响提示
- **按钮**: [保存配置] 靠左, [预览动画] 靠右

#### 会员画像页面
- **页面路径**: `/members/:id/profile`
- **页面布局**: 360度画像（左侧标签云 + 右侧详情面板）
- **核心组件**: 基础属性卡片（注册时间/活跃度/消费能力）、行为标签（兴趣偏好/互动习惯）、价值分层标签
- **交互规范**: 标签可编辑；点击标签筛选相关会员；画像数据支持导出
- **按钮**: [编辑画像] 靠左, [导出数据] 靠右
...
#### 自动续费配置页面
- **页面路径**: `/subscriptions/:id/auto-renew`
- **页面布局**: 表单 + 续费预览
- **核心组件**: 续费开关、扣款方式选择、续费提醒配置（提前3/7/15天）、异常处理策略
- **交互规范**: 续费前展示下次扣款金额和时间；异常时自动暂停并通知用户；续费成功即时推送
- **按钮**: [保存配置] 靠左, [测试扣款] 靠右
...
#### device_shares（设备分享记录表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| device_id | BIGINT FK | 设备 ID |
| owner_id | BIGINT FK | 分享人 ID |
| share_type | VARCHAR(16) | 分享类型（share/transfer） |
| share_code | VARCHAR(64) | 分享码（一次性分享） |
| status | VARCHAR(16) | 状态（active/expired/revoked） |
| expires_at | TIMESTAMP | 过期时间 |
| created_at | TIMESTAMP | 创建时间 |

#### device_share_permissions（设备分享权限表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| share_id | BIGINT FK | 分享记录 ID |
| target_user_id | BIGINT FK | 被分享用户 ID |
| permission_level | VARCHAR(16) | 权限级别（read/control/manage） |
...
#### member_active_analysis（会员活跃度分析表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| member_id | BIGINT FK | 会员 ID |
...
| total_session_minutes | INT | 总使用时长（分钟） |
| interaction_count | INT | 互动次数 |
| retention_rate | DECIMAL(5,2) | 留存率（百分比） |
| conversion_funnel | JSONB | 转化漏斗数据 |
| created_at | TIMESTAMP | 创建时间 |
...
#### subscription_auto_renew_configs（自动续费配置表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| subscription_id | BIGINT FK | 订阅 ID |
| user_id | BIGINT FK | 用户 ID |
| auto_renew_enabled | BOOLEAN | 自动续费开关 |
| payment_method | VARCHAR(32) | 扣款支付方式 |
| reminder_days | JSONB | 提前提醒天数列表（如 [3,7,15]） |
| retry_count | INT | 失败重试次数 |
|异常处理策略 | VARCHAR(32) | 策略（pause/notify/cancel） |
| last_charge_at | TIMESTAMP | 最近扣款时间 |
| next_charge_at | TIMESTAMP | 下次扣款时间 |
| status | VARCHAR(16) | 状态（active/paused/cancelled） |
| created_at | TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | 更新时间 |

#### family_plan_members（家庭计划成员表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| family_plan_id | BIGINT FK | 家庭计划 ID |
| user_id | BIGINT FK | 成员用户 ID |
| role | VARCHAR(16) | 角色（owner/member） |
| quota_limit | DECIMAL(12,2) | 个人配额上限 |
| used_quota | DECIMAL(12,2) | 已使用配额 |
| joined_at | TIMESTAMP | 加入时间 |
| left_at | TIMESTAMP | 离开时间 |
| status | VARCHAR(16) | 状态（active/inactive/removed） |
...
#### alert_escalation_rules（告警升级规则表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| rule_name | VARCHAR(128) | 规则名称 |
| alert_type | VARCHAR(64) | 告警类型 |
| condition_type | VARCHAR(32) | 条件类型（time_count/auto） |
| time_threshold_minutes | INT | 时间阈值（分钟） |
| count_threshold | INT | 次数阈值 |
| escalation_level | VARCHAR(16) | 升级目标级别 |
| escalation_channels | JSONB | 升级通知渠道 |
| enabled | BOOLEAN | 是否启用 |
| created_at | TIMESTAMP | 创建时间 |

#### alert_deduplication_rules（告警抑制规则表）
| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| rule_name | VARCHAR(128) | 规则名称 |
| dedup_key_fields | JSONB | 去重关键字段（如 alert_type + device_id + metric） |
| suppress_duration_minutes | INT | 抑制时长（分钟） |
| merge_mode | VARCHAR(16) | 合并模式（count/summary/latest） |
| enabled | BOOLEAN | 是否启用 |
...
#### devices 表 — lifecycle_status 完整状态说明

| lifecycle_status 值 | 说明 | 可转换到 |
|---------------------|------|----------|
| `pending_activation` | 待激活：设备已注册但未完成首次配对激活 | `in_service` |
| `in_service` | 服役中：设备正常运行，被用户绑定使用 | `pending_activation`, `under_repair`, `reported_loss`, `scrapped` |
| `under_repair` | 维修中：设备寄回维修，无法使用 | `in_service`, `scrapped` |
| `reported_loss` | 已挂失：设备丢失，已挂失处理 | `in_service`（找回）, `scrapped` |
| `scrapped` | 已报废：设备永久停用，不可再激活 | —（终态） |

#### ota_progress 表 — 完整字段定义

| 字段 | 类型 | 说明 |
|------|------|------|
| id | BIGINT PK | 主键 |
| deployment_id | BIGINT FK | 关联的部署任务 ID |
| device_id | BIGINT FK | 设备 ID |
| target_version | VARCHAR(32) | 目标固件版本 |
| current_version | VARCHAR(32) | 当前固件版本 |
| status | VARCHAR(16) | 升级状态：`pending`/`downloading`/`verifying`/`applying`/`completed`/`failed`/`cancelled` |
| progress_percent | INT | 下载进度（0-100） |
| error_code | VARCHAR(32) | 错误码（失败时填写） |
...
#### device_desired_state / device_reported_state 表 — sync_status 字段说明

| sync_status 值 | 说明 |
|----------------|------|
| `in_sync` | 设备状态与云端期望状态一致 |
| `pending_sync` | 有待同步的差异，等待设备上报确认 |
| `syncing` | 同步中，设备正在处理配置变更 |
| `sync_failed` | 同步失败，需要人工干预 |
| `stale` | 状态陈旧（超过 5 分钟未更新） |

#### OTA 部署任务状态（ota_deployments 表 status 字段）
...
| V2.4 | 2026-03-23 | agentcp | 补充会员等级权益配置/升级动画/活跃度分析/画像；补充设备使用统计；补充告警抑制规则；补充订阅自动续费配置和家庭计划成员表；补充8张缺失数据表；版本号V2.3→V2.4 |
| V2.5 | 2026-03-23 | agentcp | 新增MQTT协议设计章节（十一→十二）；新增关键状态机章节（九）；新增订阅权益矩阵；补充UI颜色/组件规范；补充数据模型字段（devices.lifecycle_status/ota_progress/sync_status）；版本号V2.4→V2.5 |
