<template>
  <div class="container">
    <Breadcrumb :items="['menu.pet', 'menu.pet.follow']" />
    <a-card class="general-card" title="宠物关注">
      <template #extra>
...
const columns = [
  ...
  { title: '关注用户', dataIndex: 'user_id', width: 100 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/pets/follows', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
