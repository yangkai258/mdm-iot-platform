<template>
  <div class="pro-page-container">
    ...
    <a-row :gutter="16" class="stats-row">
      <a-col :span="6">
        <a-card class="stat-card"><a-statistic title="总应用数" :value="stats.total" /></a-card>
      </a-col>
      <a-col :span="6">
        <a-card class="stat-card"><a-statistic title="已发布" :value="stats.published" :value-style="{ color: '#52c41a' }" /></a-card>
      </a-col>
      <a-col :span="6">
        <a-card class="stat-card"><a-statistic title="待审核" :value="stats.pending" :value-style="{ color: '#faad14' }" /></a-card>
      </a-col>
      <a-col :span="6">
        <a-card class="stat-card"><a-statistic title="安装次数" :value="stats.installCount" /></a-card>
      </a-col>
    </a-row>
    ...
    <!-- 创建应用抽屉 -->
    <a-drawer v-model:visible="showCreateDrawer" title="上传应用" :width="480" @before-ok="handleCreate">
      <a-form :model="createForm" layout="vertical">
        <a-form-item label="应用编码" required><a-input v-model="createForm.app_code" placeholder="如 APP001" /></a-form-item>
        <a-form-item label="应用名称" required><a-input v-model="createForm.app_name" placeholder="如 企业IM" /></a-form-item>
        ...
        <a-form-item label="Bundle ID / Package Name" required><a-input v-model="createForm.bundle_id" placeholder="如 com.company.im" /></a-form-item>
        <a-form-item label="开发者"><a-input v-model="createForm.developer" placeholder="内部开发组" /></a-form-item>
        <a-form-item label="应用描述"><a-textarea v-model="createForm.description" :rows="3" placeholder="应用功能描述" /></a-form-item>
        <a-form-item label="企业应用"><a-switch v-model="createForm.is_enterprise" /></a-form-item>
      </a-form>
    </a-drawer>

    <!-- 应用详情抽屉 -->
    <a-drawer v-model:visible="showDetailDrawer" title="应用详情" :width="520">
      <template v-if="currentApp">
        <a-descriptions :column="1" bordered size="small">
          ...
          <a-descriptions-item label="类型"><a-tag :color="getAppTypeColor(currentApp.app_type)">{{ currentApp.app_type?.toUpperCase() }}</a-tag></a-descriptions-item>
          ...
          <a-descriptions-item label="状态"><a-tag :color="getStatusColor(currentApp.status)">{{ getStatusText(currentApp.status) }}</a-tag></a-descriptions-item>
...
const filters = reactive({ appType: undefined, status: undefined, keyword: '' })
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const stats = reactive({ total: 0, published: 0, pending: 0, installCount: 0 })

const createForm = reactive({
  app_code: '', app_name: '', app_type: '', bundle_id: '', developer: '', description: '', is_enterprise: true
})
...
const loadApps = async () => {
  ...
  } catch (err) { Message.error('加载应用列表失败') }
  finally { loading.value = false }
}

const handlePageChange = (page) => { pagination.current = page; loadApps() }
const handleCreate = (done) => { done(true) }
const openDetail = (record) => { currentApp.value = record; showDetailDrawer.value = true }
const openVersions = (record) => { router.push(`/apps/versions/${record.id}`) }
const openDistribute = (record) => { router.push({ path: '/apps/distributions', query: { appId: record.id } }) }
const getAppTypeColor = (t) => ({ ios: 'pink', android: 'green', windows: 'blue', macos: 'purple' }[t] || 'gray')
const getStatusColor = (s) => ({ pending: 'yellow', approved: 'green', rejected: 'red', archived: 'gray' }[s] || 'gray')
const getStatusText = (s) => ({ pending: '待审核', approved: '已发布', rejected: '已拒绝', archived: '已归档' }[s] || s)
