<template>
  <div class="container">
    <Breadcrumb :items="['menu.platform', 'menu.platform.aiModel']" />
    <a-card class="general-card" title="AI模型版本">
      <template #extra>
        <a-space>
          <a-button type="primary" @click="openCreate"><icon-plus />新建版本</a-button>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑版本' : '新建版本'">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="模型路径"><a-input v-model="form.model_path" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', version: '', model_path: '' })
const data = ref([])
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const columns = [
  { title: '模型名称', dataIndex: 'name', width: 200 },
  { title: '版本', dataIndex: 'version', width: 120 },
  { title: '模型路径', dataIndex: 'model_path', ellipsis: true },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/platform/ai-model-versions', {
