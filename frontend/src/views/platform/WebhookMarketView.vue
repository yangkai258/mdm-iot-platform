  ...
  <div class="container">
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="URL"><a-input v-model="form.url" /></a-form-item>
        <a-form-item label="事件类型"><a-input v-model="form.event_type" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', url: '', event_type: '' })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/platform/webhook-market', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', url: '', event_type: '' }); formVisible.value = true }
