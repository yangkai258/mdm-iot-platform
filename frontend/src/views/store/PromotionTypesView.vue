  ...
  <div class="container">
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        <a-form-item label="类型名称"><a-input v-model="form.name" /></a-form-item>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/store/promotion-types', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
