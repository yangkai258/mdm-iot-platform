<template>
  <div class="container">
    <Breadcrumb :items="['menu.store', 'menu.store.locations']" />
    <a-card class="general-card" title="门店管理">
      ...
      <a-row :gutter="16">
        <a-col :span="8">
          <a-form-item label="门店名称"><a-input v-model="form.keyword" placeholder="请输入" @pressEnter="loadData" /></a-form-item>
        </a-col>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        <a-form-item label="门店名称"><a-input v-model="form.name" /></a-form-item>
...
const form = reactive({ keyword: '', name: '', address: '', phone: '' })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/store/locations', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', address: '', phone: '' }); formVisible.value = true }
