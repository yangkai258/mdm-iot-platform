  ...
  <div class="content-library">
    <a-card title="文件库">
      ...
      <a-spin :loading="loading">
        <a-table :columns="columns" :data="files" :pagination="{ pageSize: 12 }">
...
const load = async () => {
  loading.value = true
  const url = filter.value.category 
    ? `${API_BASE}/content/files?category=${filter.value.category}`
    : `${API_BASE}/content/files`
...
const download = async (record) => {
  const res = await fetch(`${API_BASE}/content/files/${record.id}/download`)
  const data = await res.json()
  window.open(data.download_url, '_blank')
}

const distribute = (record) => Message.info('分发功能开发中')
const remove = (record) => Message.success('删除成功')
