  ...
  <div class="container">
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建红包'" :width="560">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="数量"><a-input-number v-model="form.total_count" :min="1" style="width:100%" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', amount: 0, total_count: 0 })
const data = ref([])
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const columns = [
  { title: '红包名称', dataIndex: 'name', width: 200 },
  { title: '金额', dataIndex: 'amount', width: 100 },
  { title: '总数量', dataIndex: 'total_count', width: 100 },
  { title: '已领取', dataIndex: 'used_count', width: 100 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/marketing/redpackets', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
