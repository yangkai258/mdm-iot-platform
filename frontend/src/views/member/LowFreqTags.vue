<template>
  <div class="low-freq-tags-page">
    ...
    <a-alert style="margin-bottom: 16px;">
      <template #title>低频购买标签规则说明</template>
      当会员在指定时间周期内的购买次数低于阈值时，系统自动为其打上对应标签，用于识别沉睡用户。
    </a-alert>
    ...
    <a-row :gutter="16" style="margin-bottom: 16px;">
      ...
      <a-col :span="8">
        <a-card hoverable>
          <a-statistic title="低频会员数" :value="stats.memberCount || 0" />
        </a-card>
      </a-col>
      <a-col :span="8">
        <a-card hoverable>
    ...
    <a-card>
      <a-table :columns="columns" :data="ruleList" :loading="loading" row-key="id" :pagination="false">
        <template #level="{ record }">
          <a-tag color="orange">{{ record.levelName || '档位' + record.level }}</a-tag>
        </template>
      </a-table>
        <template #buyCount="{ record }">
          <span style="color: #666; font-weight: 600;">≤ {{ record.buyCount }} 次</span>
        </template>
        <template #periodDays="{ record }">
          {{ record.periodDays }} 天
        </template>
        <template #tagName="{ record }">
          <a-tag :color="record.tagColor || 'orange'">{{ record.tagName || '未设置' }}</a-tag>
        </template>
    ...
    >
      <a-form :model="form" layout="vertical">
        <a-form-item label="档位名称" :rules="[{ required: true, message: '请输入档位名称' }]">
          <a-input v-model="form.levelName" placeholder="如：低频购买（≤1次）" />
        </a-form-item>
        <a-row :gutter="12">
          <a-col :span="12">
            <a-form-item label="购买次数上限" :rules="[{ required: true, message: '请输入购买次数' }]">
              <a-input-number v-model="form.buyCount" :min="0" style="width: 100%" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
...
const columns = [
  ...
  { title: '购买次数上限', slotName: 'buyCount', width: 130 },
...
const handleToggleEnable = async (val) => {
  try {
    await api.saveLowFreqTag({ enabled: val })
...
const showCreate = () => {
  isEdit.value = false
  Object.assign(form, { levelName: '', buyCount: 1, periodDays: 90, tagId: null, tagName: '', tagColor: '#FF9800', status: 'active' })
...
const handleFormSubmit = async (done) => {
  ...
  try {
    if (isEdit.value) {
      await api.saveLowFreqTag({ id: currentId.value, ...form })
    } else {
      await api.saveLowFreqTag({ ...form })
    }
...
const handleDelete = (record) => {
  Modal.warning({
    ...
    onOk: async () => {
      try {
        await api.saveLowFreqTag({ id: record.id, _delete: true })
