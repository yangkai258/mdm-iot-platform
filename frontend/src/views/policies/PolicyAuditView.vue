<template>
  <div class="container">
    <Breadcrumb :items="['menu.policies', 'menu.policies.audit']" />
    <a-card class="general-card" title="策略变更审计">
      <template #extra>
...
const columns = [
  { title: '变更ID', dataIndex: 'id', width: 80 },
  ...
  { title: '变更时间', dataIndex: 'changed_at', width: 170 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/policies/audit', {
