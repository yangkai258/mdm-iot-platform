...
const form = reactive({ keyword: '', name: '', rule_type: '' })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/policies/compliance-rules', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', rule_type: '' }); formVisible.value = true }
