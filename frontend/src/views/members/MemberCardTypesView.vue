<template>
  <div class="container">
    <Breadcrumb :items="['menu.members', 'menu.members.cardTypes']" />
    <a-card class="general-card" title="会员卡类型">
      <template #extra>
        <a-space>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        <a-form-item label="卡类型名称"><a-input v-model="form.name" /></a-form-item>
        <a-form-item label="折扣"><a-input-number v-model="form.discount" :min="0" :max="10" :precision="2" style="width:100%" /></a-form-item>
        <a-form-item label="年费"><a-input-number v-model="form.annual_fee" :min="0" style="width:100%" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', discount: 10, annual_fee: 0 })
const data = ref([])
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const columns = [
  { title: '卡类型名称', dataIndex: 'name', width: 200 },
  { title: '折扣', dataIndex: 'discount', width: 100 },
  { title: '年费', dataIndex: 'annual_fee', width: 120 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/card-types', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', discount: 10, annual_fee: 0 }); formVisible.value = true }
