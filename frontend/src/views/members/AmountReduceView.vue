<template>
  <div class="promotion-page">
    <a-breadcrumb class="breadcrumb">
      ...
      <a-breadcrumb-item>满额减</a-breadcrumb-item>
    </a-breadcrumb>
    ...
    <a-card class="table-card">
      ...
      >
        <template #rule="{ record }">
          <span>满 <strong>¥{{ record.threshold }}</strong> 减 <strong style="color: #ff6b00;">¥{{ record.reduceAmount }}</strong></span>
        </template>
    ...
    <!-- 新建/编辑抽屉 -->
    <a-drawer v-model:visible="formVisible" :title="isEdit ? '编辑满额减活动' : '新建满额减活动'" :width="560">
        ...
        <a-row :gutter="12">
          ...
          <a-col :span="12">
            <a-form-item label="减免金额">
              <a-input-number v-model="form.reduceAmount" :min="0" :precision="2" style="width: 100%" />
            </a-form-item>
          </a-col>
        </a-row>
...
const form = reactive({
  name: '', threshold: 0, reduceAmount: 0, perLimit: 0,
...
const loadData = async () => {
  ...
  try {
    const d = {
      list: [
        { id: 1, name: '满100减20', threshold: 100, reduceAmount: 20, productName: '全场通用', dateRange: '2026-01-01 至 2026-12-31', status: 'active' },
        { id: 2, name: '满200减50', threshold: 200, reduceAmount: 50, productName: '指定商品', dateRange: '2026-03-01 至 2026-06-30', status: 'active' }
      ],
      total: 2
    }
...
const showCreateDrawer = () => {
  isEdit.value = false
  Object.assign(form, { name: '', threshold: 0, reduceAmount: 0, perLimit: 0, productIds: [], dateRange: [], description: '' })
  formVisible.value = true
}

const showEdit = (record) => {
  isEdit.value = true
  Object.assign(form, { name: record.name, threshold: record.threshold, reduceAmount: record.reduceAmount, perLimit: record.perLimit || 0, productIds: [], dateRange: [], description: record.description || '' })
  formVisible.value = true
}
