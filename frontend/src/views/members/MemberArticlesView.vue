<template>
  <div class="container">
    <Breadcrumb :items="['menu.members', 'menu.members.articles']" />
    <a-card class="general-card" title="会员文章">
      <template #extra>
        <a-space>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="600">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="内容"><a-textarea v-model="form.content" :rows="5" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', title: '', content: '' })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/articles', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { title: '', content: '' }); formVisible.value = true }
