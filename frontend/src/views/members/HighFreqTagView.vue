<template>
  <div class="container">
    <a-card class="general-card" title="高频标签">
      ...
      <a-row :gutter="16">
        <a-col :span="8">
          <a-form-item label="标签名称"><a-input v-model="form.keyword" placeholder="请输入" @pressEnter="loadData" /></a-form-item>
        </a-col>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="使用次数"><a-input-number v-model="form.use_count" :min="0" style="width:100%" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', use_count: 0 })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/high-freq-tags', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', use_count: 0 }); formVisible.value = true }
