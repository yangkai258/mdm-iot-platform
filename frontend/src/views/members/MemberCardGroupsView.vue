<template>
  <div class="container">
    <Breadcrumb :items="['menu.members', 'menu.members.cardGroups']" />
    <a-card class="general-card" title="会员卡分组">
      <template #extra>
        <a-space>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        <a-form-item label="分组名称"><a-input v-model="form.name" /></a-form-item>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/card-groups', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
