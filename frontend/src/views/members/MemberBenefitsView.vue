  ...
  <div class="container">
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'" :width="560">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="权益类型"><a-input v-model="form.benefit_type" /></a-form-item>
...
const form = reactive({ keyword: '', name: '', benefit_type: '', description: '' })
const data = ref([])
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const columns = [
  { title: '权益名称', dataIndex: 'name', width: 200 },
  { title: '权益类型', dataIndex: 'benefit_type', width: 120 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/benefits', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
    data.value = res.data?.list || []
    pagination.total = data.value.length
  } catch { data.value = [] } finally { loading.value = false }
}
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', benefit_type: '', description: '' }); formVisible.value = true }
