...
const loadPetProfile = async () => {
  try {
    ...
    Object.assign(petProfile, {
      ...
      interaction_freq: 'medium',
      dnd_enabled: true,
      dnd_start_time: dayjs('23:00', 'HH:mm'),
      dnd_end_time: dayjs('08:00', 'HH:mm')
    })
  }
}
...
const getCmdColor = (cmd) => {
  ...
  return colors[cmd] || 'default'
}

const getCmdText = (cmd) => {
  const texts = {
    reboot: '重启',
    factory_reset: '恢复出厂',
    sync_time: '同步时间',
...
onUnmounted(() => {
  if (shadowPollingTimer) {
    clearInterval(shadowPollingTimer)
  }
})
</script>
