<template>
  <div class="page-container">
    <a-card class="general-card" title="设备定位">
      <template #extra>
      ...
      <a-table :columns="columns" :data="data" :loading="loading" :pagination="pagination" @page-change="onPageChange" row-key="id">
        ...
        <template #actions="{ record }">
          ...
          <a-button v-if="record.status === 'lost'" type="primary" status="success" size="small" @click="handleFound(record)">已寻回</a-button>
        </template>
      </a-table>
    </a-card>
  </div>
...
const columns = [
  ...
  { title: '最后位置', dataIndex: 'last_location', width: 200 },
  { title: '最后在线', dataIndex: 'last_online', width: 160 },
...
const getStatusText = (status) => {
  const texts = { online: '在线', offline: '离线', lost: '丢失', found: '已寻回' }
...
const loadMockData = () => {
  const mockData = {
    online: [
      ...
      { id: 2, device_id: 'DEV002', pet_name: '旺财', last_location: '上海市浦东新区', last_online: '2026-04-09 19:25:00', battery: '72%', status: 'online' }
    ],
    lost: [
...
const handleFound = (record) => {
  record.status = 'found'
  Message.success(`已将 ${record.pet_name} 标记为已寻回`)
}
