<template>
  <div class="pro-page-container">
    <a-breadcrumb class="pro-breadcrumb">
      ...
      <a-breadcrumb-item>设备配对</a-breadcrumb-item>
    </a-breadcrumb>

    <div class="pro-search-bar">
      <a-space>
        <a-input-search v-model="searchKeyword" placeholder="搜索设备ID/用户" style="width: 280px" @search="loadRequests" search-button />
        <a-select v-model="filterStatus" placeholder="选择状态" allow-clear style="width: 130px" @change="loadRequests">
    ...
    <a-modal v-model:visible="rejectModalVisible" title="拒绝配对请求" @ok="handleReject" :width="480" :loading="submitting">
      <a-form layout="vertical">
        ...
        <a-form-item label="拒绝原因" required>
          <a-select v-model="rejectReason" placeholder="选择或输入拒绝原因">
            <a-option value="unauthorized">未授权该设备</a-option>
        ...
        <a-form-item label="备注">
          <a-textarea v-model="rejectNote" placeholder="可选说明" :rows="2" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const columns = [
  ...
  { title: '申请人', dataIndex: 'user_name', width: 120 },
...
const getStatusText = (s) => ({ pending: '待审核', approved: '已批准', rejected: '已拒绝', expired: '已过期' }[s] || s)
