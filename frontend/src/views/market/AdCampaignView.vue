<template>
  <div class="container">
    <Breadcrumb :items="['menu.market', 'menu.market.campaign']" />
    <a-card class="general-card" title="广告活动">
      <template #extra>
        <a-space>
          <a-button type="primary" @click="openCreate"><icon-plus />新建活动</a-button>
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑活动' : '新建活动'" :width="560">
      <a-form :model="form" layout="vertical">
        <a-form-item label="活动名称"><a-input v-model="form.name" /></a-form-item>
        <a-form-item label="活动预算"><a-input-number v-model="form.budget" :min="0" style="width: 100%" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', name: '', budget: 0 })
const data = ref([])
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/market/campaigns', {
...
const openCreate = () => { isEdit.value = false; Object.assign(form, { name: '', budget: 0 }); formVisible.value = true }
