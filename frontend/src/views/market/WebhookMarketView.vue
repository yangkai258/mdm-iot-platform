    ...
    <a-card class="general-card" title="Webhooks模板市场">
        ...
        <a-form :model="form" layout="inline">
          ...
          <a-form-item label="分类">
            <a-select v-model="form.category" placeholder="请选择" allow-clear style="width: 140px">
              ...
              <a-option value="automation">自动�?/a-option>
      ...
      <a-table :columns="columns" :data="data" :loading="loading" :pagination="pagination" @page-change="onPageChange" row-key="id">
        <template #actions="{ record }">
          <a-button type="primary" size="small" @click="handleInstall(record)">一键安�?/a-button>
        </template>
      </a-table>
    </a-card>
  </div>
...
const columns = [
  ...
  { title: '作�?, dataIndex: 'author', width: 120 },
...
const loadMockData = () => {
  data.value = [
    { id: 1, name: '设备离线通知', description: '当设备离线时自动发送通知到企业微�?, category: 'notification', category_name: '通知�?, usage_count: 1250, author: '官方' },
    ...
    { id: 4, name: '定时健康检�?, description: '每天定时检查设备健康状�?, category: 'automation', category_name: '自动�?, usage_count: 780, author: '官方' },
...
const handleInstall = (record) => {
  Message.success(`已安装模�? ${record.name}`)
}
