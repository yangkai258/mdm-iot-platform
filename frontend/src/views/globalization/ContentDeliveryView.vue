  ...
  <div class="container">
    ...
    <a-modal v-model="formVisible" :title="isEdit ? '编辑' : '新建'">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item label="源区域"><a-input v-model="form.source_region" /></a-form-item>
        <a-form-item label="目标区域"><a-input v-model="form.target_region" /></a-form-item>
      </a-form>
      <template #footer>
...
const form = reactive({ keyword: '', rule_name: '', source_region: '', target_region: '' })
const data = ref([])
const pagination = reactive({ current: 1, pageSize: 20, total: 0 })
const columns = [
  { title: '规则名称', dataIndex: 'rule_name', width: 200 },
  { title: '源区域', dataIndex: 'source_region', width: 160 },
  { title: '目标区域', dataIndex: 'target_region', width: 160 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/globalization/content-delivery', {
...
const openCreate = () => { isEdit.value = false; Object.assign(form, { rule_name: '', source_region: '', target_region: '' }); formVisible.value = true }
