<template>
  <div class="container">
    <a-card class="general-card" title="区域AI节点">
      ...
      <a-row :gutter="16">
        <a-col :span="8">
          <a-form-item label="节点名称"><a-input v-model="form.keyword" placeholder="请输入" @pressEnter="loadData" /></a-form-item>
        </a-col>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/globalization/regional-ai-nodes', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
