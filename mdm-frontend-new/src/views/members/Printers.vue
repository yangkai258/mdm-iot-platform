<template>
  <div class="pro-page-container">
    ...
    <div class="pro-search-bar">
      <a-space wrap>
        <a-input-search v-model="filters.keyword" placeholder="搜索设备名称/编号" style="width: 240px" search-button @change="loadData" />
    ...
    <div class="pro-action-bar">
      <a-space>
        <a-button type="primary" @click="openCreate">添加设备</a-button>
    ...
    <a-modal v-model:visible="modalVisible" :title="isEdit ? '编辑设备' : '添加设备'" @ok="handleSubmit" :width="520" :mask-closable="false">
...
const columns = [
  ...
  { title: '连接状态', slotName: 'onlineStatus', width: 100 },
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const data = await res.json()
    if (data.code === 0) { Message.success(isEdit.value ? '更新成功' : '添加成功'); modalVisible.value = false; loadData() }
