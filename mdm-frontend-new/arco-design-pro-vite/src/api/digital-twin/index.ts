...
const BASE_URL = '/api/digital-twin'
...
const digitalTwinApi = {
  ...
  getCurrentVitals: (deviceId: string) => {
    return axios.get(`/api/vitals/current/${deviceId}`, { headers: headers() }).then(r => r.data)
  },

  // 获取生命体征趋势（最近24小时）
  getVitalsTrend: (deviceId: string, params?: { hours?: number }) => {
    return axios.get(`/api/vitals/trend/${deviceId}`, { params, headers: headers() }).then(r => r.data)
  },
  ...
  getHistoryEvents: (deviceId: string, params?: {
    ...
    return axios.get(`/api/history/events/${deviceId}`, { params, headers: headers() }).then(r => r.data)
  },

  // 获取行为预测
  getBehaviorPrediction: (deviceId: string) => {
    return axios.get(`/api/behavior/prediction/${deviceId}`, { headers: headers() }).then(r => r.data)
  },

  // 获取宠物列表（用于选择）
  getPets: (params?: { page?: number; page_size?: number }) => {
    return axios.get(`/api/pets`, { params, headers: headers() }).then(r => r.data)
  }
}
