<template>
  <div class="pet-config-view-container">
    <Breadcrumb :items="[{title: '首页', path: '/'},{title: '宠物管理'},{title: '宠物配置'}]" />
...
const searchForm = reactive({ keyword: '', personality: undefined as number | undefined })
const pets = ref<any[]>([])
...
const petForm = reactive({
  ...
  dnd_enabled: false, dnd_start_time: null as any, dnd_end_time: null as any,
...
const onPageChange = (page: number) => { pagination.current = page; loadPets() }
const onPageSizeChange = (size: number) => { pagination.pageSize = size; pagination.current = 1; loadPets() }
...
const editPet = (record: any) => {
  isEdit.value = true
  Object.assign(petForm, {
    ...record,
    dnd_start_time: record.dnd_start_time ? record.dnd_start_time : null,
    dnd_end_time: record.dnd_end_time ? record.dnd_end_time : null,
...
const handleSave = async () => {
  ...
  try {
    ...
    const submitData = {
      ...petForm,
      dnd_start_time: petForm.dnd_start_time?.format?.('HH:mm') || petForm.dnd_start_time || '',
      dnd_end_time: petForm.dnd_end_time?.format?.('HH:mm') || petForm.dnd_end_time || ''
    }
  ...
  if (isEdit.value) {
    ...
    pets.value.unshift({ pet_id: `PET${Date.now()}`, ...petForm, create_time: new Date().toLocaleString(), avatar_color: ['#f6ad55', '#fc8181', '#68d391', '#63b3ed', '#f687b3'][Math.floor(Math.random() * 5)] })
...
const deletePet = (record: any) => { pets.value = pets.value.filter(p => p.pet_id !== record.pet_id); Message.success('宠物已删除') }
...
const getPersonalityColor = (p: number) => ({ 1: 'green', 2: 'blue', 3: 'purple', 4: 'orange', 5: 'pink' }[p] || 'default')
const getPersonalityText = (p: number) => ({ 1: '活泼好动', 2: '温顺安静', 3: '好奇宝宝', 4: '独立自主', 5: '粘人依赖' }[p] || '未知')

onMounted(() => { loadPets(); loadDevices() })
</script>

<style scoped>
.pet-config-view-container {
  padding: 20px 24px;
