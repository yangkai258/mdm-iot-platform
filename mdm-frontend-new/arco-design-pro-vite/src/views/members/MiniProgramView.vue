...
const columns = [
  { title: '视图名称', dataIndex: 'name', width: 180 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/members/mini-program-views', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
