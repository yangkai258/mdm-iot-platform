...
const columns = [
  { title: '优惠券ID', dataIndex: 'id', width: 80 },
  { title: '名称', dataIndex: 'name', width: 200 },
  { title: '面额', dataIndex: 'amount', width: 100 },
  { title: '有效期', dataIndex: 'validity', width: 160 },
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await fetch('/api/v1/marketing/temp-coupons', { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } }).then(r => r.json())
