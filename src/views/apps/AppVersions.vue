<template>
  <a-layout class="app-versions">
        ...
        <a-card class="version-card">
          ...
          >
            <template #version="{ record }">
              <a-tag :color="record.is_latest ? 'blue' : 'gray'">
                {{ record.version }}
                <span v-if="record.is_latest"> (最新)</span>
              </a-tag>
            </template>
            <template #mandatory="{ record }">
              <a-tag :color="record.is_mandatory ? 'orange' : 'default'">
                {{ record.is_mandatory ? '强制更新' : '可选更新' }}
              </a-tag>
            </template>
    ...
    <!-- 上传版本抽屉 -->
    <a-drawer
      v-model:visible="showUploadDrawer"
      title="上传新版本"
      :width="480"
    >
      <a-form :model="uploadForm" layout="vertical" @submit-success="handleUploadSubmit">
        <a-form-item label="版本号" required>
          <a-input v-model="uploadForm.version" placeholder="如 v2.1.0" />
        </a-form-item>
        <a-form-item label="版本数字" required>
          <a-input-number v-model="uploadForm.version_code" :min="1" placeholder="如 21" style="width: 100%" />
        </a-form-item>
        <a-form-item label="安装包 URL" required>
          <a-input v-model="uploadForm.file_url" placeholder="https://cdn.example.com/app-v2.1.0.apk" />
        </a-form-item>
        <a-form-item label="文件大小 (字节)">
          <a-input-number v-model="uploadForm.file_size" :min="0" placeholder="如 52428800" style="width: 100%" />
        </a-form-item>
        <a-form-item label="MD5 校验码">
          <a-input v-model="uploadForm.file_md5" placeholder="文件 MD5 校验码" />
        </a-form-item>
        <a-form-item label="最低系统版本">
          <a-input v-model="uploadForm.min_os_version" placeholder="如 14.0" />
        </a-form-item>
        <a-form-item label="更新日志">
          <a-textarea v-model="uploadForm.release_notes" :rows="4" placeholder="本次更新内容..." />
        </a-form-item>
        <a-form-item label="强制更新">
          <a-switch v-model="uploadForm.is_mandatory" />
        </a-form-item>
        <a-form-item>
          <a-space>
    ...
    >
      <template v-if="currentVersion">
        <a-descriptions :column="1" bordered size="small">
          ...
          <a-descriptions-item label="安装包 URL">
            <a :href="currentVersion.file_url" target="_blank">{{ currentVersion.file_url }}</a>
          </a-descriptions-item>
          <a-descriptions-item label="强制更新">
            <a-tag :color="currentVersion.is_mandatory ? 'orange' : 'default'">
              {{ currentVersion.is_mandatory ? '是' : '否' }}
            </a-tag>
          </a-descriptions-item>
          <a-descriptions-item label="最新版本">
            <a-tag :color="currentVersion.is_latest ? 'blue' : 'default'">
              {{ currentVersion.is_latest ? '是' : '否' }}
            </a-tag>
          </a-descriptions-item>
...
const resetUploadForm = () => {
  ...
  uploadForm.file_url = ''
  uploadForm.file_size = null
  uploadForm.file_md5 = ''
  uploadForm.min_os_version = ''
  uploadForm.release_notes = ''
  uploadForm.is_mandatory = false
}
