<template>
  <div class="occupation-types-page">
    ...
    <a-card class="action-card">
      <a-space wrap>
        <a-input-search v-model="filters.keyword" placeholder="搜索从业类型名称/编码" style="width: 260px" search-button @search="handleSearch" />
    ...
    <!-- 详情抽屉 -->
    <a-drawer v-model:visible="detailVisible" title="从业类型详情" :width="480">
      <template v-if="currentRecord">
        <a-descriptions :column="1" bordered size="small">
          <a-descriptions-item label="从业类型名称">{{ currentRecord.name }}</a-descriptions-item>
          <a-descriptions-item label="从业类型编码">{{ currentRecord.code }}</a-descriptions-item>
          <a-descriptions-item label="排序">{{ currentRecord.sort || 0 }}</a-descriptions-item>
    ...
    >
      <a-form :model="form" layout="vertical">
        <a-form-item label="从业类型名称" field="name" :rules="[{ required: true, message: '请输入从业类型名称' }]">
          <a-input v-model="form.name" placeholder="如：宠物美容师" />
        </a-form-item>
        <a-form-item label="从业类型编码" field="code" :rules="[{ required: true, message: '请输入从业类型编码' }]">
          <a-input v-model="form.code" placeholder="如：BEAUTICIAN" :disabled="isEdit" />
        </a-form-item>
        <a-form-item label="排序值" field="sort">
          <a-input-number v-model="form.sort" :min="0" style="width: 100%" />
        </a-form-item>
        <a-form-item label="描述">
          <a-textarea v-model="form.description" :rows="3" placeholder="简要描述该从业类型" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const columns = [
  { title: '从业类型名称', dataIndex: 'name', width: 180 },
  { title: '从业类型编码', dataIndex: 'code', width: 160 },
...
const handleFormSubmit = async (done) => {
  ...
  try {
    if (isEdit.value) { await api.updateOccupationType(currentId.value, { ...form }); Message.success('更新成功') }
    else { await api.createOccupationType({ ...form }); Message.success('创建成功') }
...
const handleDelete = (record) => {
  Modal.warning({
    title: '确认删除', content: `确定要删除从业类型「${record.name}」吗？`,
    okText: '确认删除', onOk: async () => {
      try { await api.deleteOccupationType(record.id); Message.success('删除成功'); loadData() }
...
<style scoped>
.occupation-types-page { padding: 20px 24px; min-height: calc(100vh - 64px); background: #f5f7fa; }
