...
const BASE_URL = '/api/v1/tenant-approvals'
...
export async function approveApplication(id, comment = '') {
  const res = await axios.post(`${BASE_URL}/${id}/approve`, { comment }, { headers: headers() })
  return res.data
}
...
export async function rejectApplication(id, comment = '') {
  const res = await axios.post(`${BASE_URL}/${id}/reject`, { comment }, { headers: headers() })
  return res.data
}
...
export async function getApprovalHistory(id) {
  const res = await axios.get(`${BASE_URL}/${id}/history`, { headers: headers() })
  return res.data
}
